...
#' @noRd
db_con <- function() {
  if (is.null(.pkg_env$con) || !DBI::dbIsValid(.pkg_env$con)) {
    .pkg_env$con <- DBI::dbConnect(
      drv     = duckdb::duckdb(),
      dbdir   = ":memory:"
    )
    ...
    reg.finalizer(
      ...
      f        = \(env) {
        try(
          expr   = DBI::dbDisconnect(conn = env$con, shutdown = TRUE),
          silent = TRUE
        )
      },
      onexit   = TRUE
...
db_table_name <- function(prefix, args_list) {
  h <- substr(
    x     = digest::digest(object = args_list, algo = "md5"),
    start = 1L,
    stop  = 6L
  )
  paste0(prefix, "_", h)
...
#'
#' @noRd
.format_envelope <- function(table, n_rows, df_preview, columns, hint,
  ...
  preview_csv <- paste(
    utils::capture.output(
      utils::write.csv(x = df_preview, file = stdout(), row.names = FALSE)
    ),
    collapse = "\n"
  )
...
.detect_multi_series <- function(con, table) {
  cols <- DBI::dbListFields(conn = con, name = table)
  candidates <- c(
  ...
  for (col in cols_to_check) {
    qcol <- DBI::dbQuoteIdentifier(conn = con, x = col)
    qtab <- DBI::dbQuoteIdentifier(conn = con, x = table)
    vals <- DBI::dbGetQuery(
      conn      = con,
      statement = sprintf(
...
.build_hint <- function(table, df) {
  ts_col <- names(df)[vapply(
    X         = df,
    FUN       = \(c) inherits(c, "POSIXct") || inherits(c, "Date"),
    FUN.VALUE = logical(1L)
  )]
  num_col <- names(df)[vapply(
    X         = df,
    FUN       = \(c) {
      is.numeric(c) && !inherits(c, "POSIXct") && !inherits(c, "Date")
    },
    FUN.VALUE = logical(1L)
  )]
...
db_store <- function(df, prefix, args_list, preview_rows = 5L) {
  ...
  if (!DBI::dbExistsTable(conn = con, name = table)) {
    DBI::dbWriteTable(
      conn      = con,
      name      = table,
      value     = df,
...
db_query <- function(sql, max_rows = 100L) {
  con <- db_con()
  result <- tryCatch(
    expr  = DBI::dbGetQuery(conn = con, statement = sql),
    error = \(e) e
  )
  ...
  out <- paste(
    utils::capture.output(
      utils::write.csv(x = result, file = stdout(), row.names = FALSE)
    ),
    collapse = "\n"
  )
  if (truncated) {
...
db_list <- function() {
  ...
  cols_per <- vapply(
    X         = tabs,
    FUN       = \(t) {
      fields <- DBI::dbListFields(conn = con, name = t)
  ...
  paste(
    utils::capture.output(
      utils::write.csv(x = df, file = stdout(), row.names = FALSE)
    ),
    collapse = "\n"
  )
}
...
db_describe <- function(name, n_preview = 5L) {
  con <- db_con()
  if (!DBI::dbExistsTable(conn = con, name = name)) {
